...
*  `file` = The full path to the file on your computer as a text string. (Use the forward slash `/`, even on Windows.) A more convient alternative is to use `file.choose()` to select the file using the operating system file chooser.
...
Usually, I would recommend not writing out modified data frames to text files.  Far better to have a single input file, and to do all necessary manipulations in your saved R script file. But, if you have huge datasets or need to use another statistical prigem, it is sometimes necesary to save a dataframe out as a text file.  You can do this with `write.table()`
...
First, I add a new column to the femur length dataset and then write it out to the Desktop.
